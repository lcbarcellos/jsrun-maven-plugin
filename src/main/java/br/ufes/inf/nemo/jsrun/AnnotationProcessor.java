...
public class AnnotationProcessor extends AbstractProcessor {
    ...
    public boolean process(Set<? extends TypeElement> set, RoundEnvironment re) {
        ...
        if (scriptRunner == null) {
            ...
            if (set != null) {
                elementVisitor.array();
                for (TypeElement annotationType : set) {
                    for (Element annotatedType : re.getElementsAnnotatedWith(annotationType)) {
                        if (annotatedType instanceof TypeElement) {
                            elementVisitor.add((TypeElement) annotatedType);
                        }
                    }
                }
                elementVisitor.endArray();
            } else {
...
class ElementVisitor {
    ...
    public void add(TypeElement typeElement) {
        if (visited.add(typeElement)) {
            configWriter.object()
                .key("typeName").value(typeElement.getQualifiedName().toString())
